...
def _resolve_value(
    ...
    *,
    ...
    if value.mode == OptimizationValueMode.fixed:
...
def _parameter_entry(
    ...
    *,
    ...
    if (
        ...
        and distribution != StandardDistanceDependentDistributionName.uniform
    ):
...
def _build_global_parameters(
    ...
    for name in sorted(selection.global_parameters):
        ...
        if selected.ion_channel_model is not None:
            ...
            if variable is None or variable.variable_type != VariableType.global_:
                msg = (
        ...
        for assignment in selection.mechanism_regions[location]:
            ...
            for name in sorted(assignment.parameters):
                ...
                if variable is None:
                    ...
                    raise ValueError(msg)
                if variable.variable_type != VariableType.range:
                    msg = (
...
def _validate_myelinated_capability(
    ...
    if capabilities.has_myelinated is True:
        return
    uses_myelinated = (
        SectionListName.myelinated in selection.base_parameters
        or SectionListName.myelinated in selection.mechanism_regions
    )
...
def build_params_definition(
    ...
    *,
    ...
    """Compile a complete BluePyEModel params definition from local preprocessing inputs.
    ...
    custom ``distance_dependent_distributions`` declarations.
    """
    selection = params_input.parameters_selection
    custom_distributions = params_input.distance_dependent_distributions
    ...
    used_distributions = {StandardDistanceDependentDistributionName.uniform}
